...
function UserNavBar() {
    ...
    const { isLoggedIn, isUserLoggedIn, isAdminLoggedIn, isVendorLoggedIn, isCourierLoggedIn } = useUserstate();
    console.log(isLoggedIn, isUserLoggedIn, isAdminLoggedIn, isVendorLoggedIn, isCourierLoggedIn);
    ...
    return (
        <div>
            {isLoggedIn && <nav className="flex justify-between items-center p-4 bg-gray-800 text-white relative">
                <div className="relative cursor-pointer lg:hidden" onMouseEnter={toggleDropdown} onMouseLeave={toggleDropdown}>
                    ...
                    <div className={`absolute top-full left-0 flex-col bg-gray-800 list-none min-w-[200px] z-10 shadow-lg ${dropdownOpen ? "flex" : "hidden"}`} onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
                        <Link className="block bg-gray-700 text-white p-2 mb-1 rounded-lg transition duration-300 ease-in-out transform hover:bg-gray-600 hover:text-yellow-500 hover:scale-105 shadow-sm" to="/login">Login</Link>
                        <Link className="block bg-gray-700 text-white p-2 mb-1 rounded-lg transition duration-300 ease-in-out transform hover:bg-gray-600 hover:text-yellow-500 hover:scale-105 shadow-sm" to="/register">Register</Link>
                    </div>
                </div>
                <h1 className="m-0 text-xl text-right">ShopSmartly</h1>
                <div className="hidden lg:flex space-x-4">
                    <Link className="text-white p-2 transition duration-300 ease-in-out transform hover:bg-gray-600 hover:text-yellow-500 hover:scale-105" to="/login">Login</Link>
                    <Link className="text-white p-2 transition duration-300 ease-in-out transform hover:bg-gray-600 hover:text-yellow-500 hover:scale-105" to="/register">Register</Link>
                </div>
            </nav>}
            {isUserLoggedIn &&
